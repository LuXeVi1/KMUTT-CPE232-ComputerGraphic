...
Promise.all([
  ...
  function draw() {
    ...
    let scaleMatrix = Mat3.scaling(scale, scale);
    let rotationMatrix = Mat3.rotation(angle);
    let translationMatrix = Mat3.translation(posX, posY);

    // Multiply matrices: T * R * S
    // Note: multiply(a, b) results in a * b.
    // We want T * (R * S).
    let tempMatrix = Mat3.multiply(rotationMatrix, scaleMatrix);
    let matrix = Mat3.multiply(translationMatrix, tempMatrix);
